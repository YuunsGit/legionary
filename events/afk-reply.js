    ...
    execute(message, client) {
        ...
        message.mentions.members.forEach((user) => {
            ...
            if (user.roles.cache.has("1013114060415434833")) {
                reaction = `${user.displayName} şu anda yemekte, bilgilendirmiş olayım. ${Util.emoji(
                    "ln_pepefull",
                    client
                )}`;
            } else if (user.roles.cache.has("1013116388451303505")) {
                reaction = `${user.displayName} oldukça meşgul. Rahatsız etmek istemezsin. ${Util.emoji(
                    "ln_pepethink",
                    client
                )}`;
            } else if (user.roles.cache.has("1013114184650731535")) {
                reaction = `${user.displayName} şu anda derste, haber vereyim dedim. ${Util.emoji(
                    "ln_pepesmart",
                    client
                )}`;
            } else if (user.roles.cache.has("1013115060811485255")) {
                reaction = `${user.displayName} şu anda evde değil, haberin olsun. ${Util.emoji(
                    "ln_pepecool",
                    client
                )}`;
