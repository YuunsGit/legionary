...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const wordObj = await fetch(encodeURI(`http://localhost:8080/translate?phrase=${word}`)).then(res => res.json()).then(out => {
            return out
        })
        ...
        if (wordObj.phrases.length > 10) desc = `${desc}\n[... ${wordObj.phrases.length-10} sonuç daha](${encodeURI(`https://tureng.com/tr/turkce-ingilizce/${word}`)})`
