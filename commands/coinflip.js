...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (talkedRecently.has(interaction.member.id)) {
            ...
            const memberObject = Util.getMember(interaction.member.id)
            ...
            if (random === 525) {
                embed.setTitle('Dik')
                    ...
                    .setDescription(`Az önce bir parayı hava atıp\nyere dik düşürmeyi başardın!\nÖdül olarak tam 999${Util.emoji('LP_small_spin', interaction.client)} kazandın!`)
                memberObject.lp += 999
            ...
            switch (value) {
                case 0:
                    embed.setTitle('Yazı')
                        .setThumbnail('attachment://yazi.png')
                    await interaction.reply({embeds: [embed], files: [yazi], allowedMentions: {repliedUser: true}})
                    break
                case 1:
                    embed.setTitle('Tura')
                        .setThumbnail('attachment://tura.png')
                    await interaction.reply({embeds: [embed], files: [tura], allowedMentions: {repliedUser: true}})
                    break
                default:
                    embed.setTitle('Garip bir şeyler oldu')
                        .setDescription('Bir hata nedeniyle sonuç bulunamadı.')
            }

            if (side) {
                if (side.toLowerCase() === embed.title.toLowerCase() || side.toLowerCase() === embed.title.toLowerCase().charAt(0)) {
                    switch (embed.title.toLowerCase()) {
                        case 'dik':
                            interaction.editReply({content: `${Util.emoji('LN_trigger', interaction.client)} İddiayı kazandın! Ödülün tam **1000**${Util.emoji('LP_small_spin', interaction.client)}`})
                            memberObject.lp += 1000
                            break
                        default:
                            interaction.editReply({content: `${Util.emoji('LN_trigger', interaction.client)} İddiayı kazandın! Ödülün tam **25**${Util.emoji('LP_small_spin', interaction.client)}`})
                            memberObject.lp += 25
                    }
                } else {
                    interaction.editReply({content: `:moyai: İddiayı kaybettin. **50**${Util.emoji('LP_small_spin', interaction.client)} hesabından alındı.`})
                    memberObject.lp -= 50
                }
            }
