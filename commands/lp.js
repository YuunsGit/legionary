    ...
    async execute(interaction) {
        ...
        switch (action) {
            case "ekle":
                ...
                interaction.reply({
                    content: `**${count}${Util.emoji(
                        "LP_small_spin",
                        interaction.client
                    )}** kullanıcı hesabına eklendi.`,
                });
                break;
            case "sil":
                memberObject.lp -= count;
                interaction.reply({
                    content: `**${count}${Util.emoji(
                        "LP_small_spin",
                        interaction.client
                    )}** kullanıcı hesabından silindi.`,
                });
                break;
            case "yolla":
                if (memberObjectSelf.lp < count) {
                    interaction.reply({
                        content: `Hesabında **${count}${Util.emoji("LP_small_spin", interaction.client)}** bulunmuyor.`,
                    });
                    return;
                }
                ...
                const embed = new MessageEmbed()
                    ...
                    .setDescription(
                        `> **Gönderen: ${interaction.member.toString()}** 
