...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let gif = await petpet(interaction.options.getUser('kullanıcı').displayAvatarURL({format: "png"}), {delay: 30})
        ...
        await interaction.reply({content: `> ${interaction.options.getUser('kullanıcı').toString()}`,embeds: [embed], files: [attachment]})
