    ...
    async execute(interaction) {
        ...
        switch (action) {
            case 'bağekle':
                ...
                interaction.reply({content: `${second} bağlantısı profiline başarıyla eklendi. ${Util.emoji('ln_pepeok', interaction.client)}`})
                break
            ...
            case 'bağsil':
                ...
                for (const one of memberObject.links) {
                    if (one.name.toLowerCase() === second.toLowerCase()) {
                        ...
                        interaction.reply({content: `${second} bağlantısı profilinden silindi. ${Util.emoji('ln_pepeok', interaction.client)}`})
                        return
                    }
                    a++
                }
                interaction.reply({
                    content: `Bu isme sahip bir bağlantı platformu bulunamadı. ${Util.emoji('ln_pepezoom', interaction.client)}`,
                    ephemeral: true
                })
                break
            ...
            case 'dgünü':
                ...
                interaction.reply({content: `Doğum günün başarıla ${second} olarak ayarlandı. ${Util.emoji('ln_pepeok', interaction.client)}`})
                break
