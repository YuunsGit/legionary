    ...
    execute(interaction) {
        ...
        switch (action) {
            ...
            case 'sil':
                if (!third) {
                    interaction.reply({
                        content: `Silinecek kaydın ID'sini de girmen gerekli. ${Util.emoji('ln_pepezoom', interaction.client)}`,
                        ephemeral: true
                    })
                    return
                }
                ...
                for (const day of logs[genre]) {
                    if (day.id === logID) {
                        ...
                        interaction.reply(`${logID} ID'sine sahip kayıt silindi. ${Util.emoji('ln_pepeok', interaction.client)}`)
                        return
                    }
                    a++
                }
            ...
            case 'ekle':
                if (!third) {
                    interaction.reply({
                        content: `Eklenecek kaydı belirtmen gerekiyor. ${Util.emoji('ln_pepezoom', interaction.client)}`,
                        ephemeral: true
                    })
                    return
                }
                const newLog = {
                    ...
                    "date": interaction.createdAt.getUTCDate() + '.' + (interaction.createdAt.getUTCMonth() + 1) + '.' + interaction.createdAt.getUTCFullYear()
                }
