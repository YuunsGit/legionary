...
module.exports = {
    ...
    async execute(interaction) {
        if (interaction.channel.id !== '935543376126832700') {
            interaction.reply({content: `O komudun yeri burası değil, yallah ${interaction.guild.channels.cache.get('935543376126832700').toString()} kanalına.`})
            return
        }
        ...
        if (memberObjectSelf.lp < bet) {
            interaction.reply({content: `Bu bahsi koyacak kadar paran yok. ${Util.emoji('ln_pepecray', interaction.client)}`})
            return
        }

        if (memberObject.lp < bet) {
            interaction.reply({content: `Rakibinin bu bahsi kabul edecek kadar parası yok. ${Util.emoji('ln_pepecray', interaction.client)}`})
            return
        }
        ...
        const betDesc = bet ? `**Bahis miktarı ${bet}${Util.emoji('LP_small_spin', interaction.client)}**` : '**Bahis yok**'
        ...
        const collector = await sentEmbed.createMessageComponentCollector({componentType: 'BUTTON', time: 3*60*1000})
        ...
        collector.on('collect', async i => {
            switch (i.customId) {
                ...
                case 'refuse':
                    if (i.member.id !== user.id) break
                    await interaction.editReply({content: '✋🏽 *Düello teklifi reddedildi.*', embeds: [], components: []})
                    collector.stop('butt')
                    break
